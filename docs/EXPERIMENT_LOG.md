...
- Seed: 42
- Batch size: 16
- Preprocessing: `tf.keras.applications.mobilenet_v2.preprocess_input` (Lambda) after augmentation, before the backbone (tf mode: [0, 255] -> [-1, 1])
- Assumption: keras.applications.MobileNetV2 does not embed a Rescaling layer (unlike EfficientNet/MobileNetV3); verify first base-model layer names at runtime
